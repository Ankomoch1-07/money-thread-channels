...
COMMON_RULES = (
    "- 話者名は必ず次の7名のみを一字一句そのまま使う（改変・別名・誤字は禁止）：\n"
    "  四国めたん / ずんだもん / 玄野武宏 / 青山龍星 / 九州そら / 春日部つむぎ / No.7。\n"
    "  （No.7＝養分ニキ＝否定・煽り役。語尾にｗを付ける）\n"
    "- 数値は必ず上の fact 値を使い、直後に [要ファクトチェック] を付ける。\n"
    "- 「最高値更新か」が「いいえ」の場合、「最高値更新」と書かない（事実に反するため）。\n"
    "- 各掛け合いブロックは最低8〜12レス。薄く終わらせない。\n"
    "- [IMG:key]/[OPIMG:]/[EDIMG:] は下記『使えるいらすとや素材』の key だけ（存在しないkeyを創作しない）。1動画20点以内。\n"
    "- グラフは必ず独立した1行で、数値付きで書く：\n"
    "  [GRAPH: タイトル | ラベルA:数値, ラベルB:数値 | Y軸ラベル]\n"
    "  ・ラベルにコロン(:)や『高→金』のような記号を入れない（値の区切りと混同するため）。値は必ず半角数字。\n"
    "  ・セリフ行(【話者名】…)の中に [GRAPH:] を埋め込まない（拾われず表示されない）。\n"
    "  ・良い例: [GRAPH: 実質金利と金価格の関係 | 金利低下局面:80, 金利上昇局面:35 | 金が買われた割合(%)]\n"
    "  ・悪い例: [GRAPH: … | 実質金利:高→金:弱, 実質金利:低→金:強 | …]（値が文字＋ラベルにコロン→グラフが空になる）\n"
    "- 「フィクション」「創作」「投資助言ではありません」等の断り書きは台本本文に一切書かない"
    "（動画に入れない。方針としては守るが、その旨をセリフに出さない。特に冒頭OP_DESCに書かない）。\n"
    "- 出力は台本本文（マーカー行と【話者名】…）のみ。前置き・後書き・説明文は書かない。\n"
)

# 動画演出フォーマット（channel_03準拠）。マルチパスの第1部/第3部の指示に合成する。
VIDEO_FORMAT_OP = (
    "【冒頭は必ず次の順で書く（動画のOP画面を作るため）】\n"
    "1行目: # 【2chゴールドスレ】＋フック＋【2ch有益スレ】（タイトル）\n"
    "[OP_HOOK]\n【四国めたん】今回ご紹介するスレッドはこちら。\n"
    "[OP_TITLE]\n[OPIMG: 素材key3つカンマ区切り（スレ内容に合うもの）]\n【四国めたん】(スレタイを1文で)\n"
    "[OP_DESC]\n【四国めたん】(このスレで何が語られるかの説明。2〜4文。最後は『それでは早速見ていきましょう。』)\n"
    "以降が本編。場面の感情が変わるレスの直前に [IMG:感情key] を置く（sticky＝次の[IMG:]まで継続）。"
    "パニック/焦り=money_shock_man等, 喜び/納得=gutspose等, 不安=komatta_man等（下記素材参照）。\n"
    "解説の数値は [GRAPH: タイトル | ラベル:値, ラベル:値 | Y軸ラベル] の形式で1行。\n"
)
VIDEO_FORMAT_ED = (
    "【最後は必ず次の順で締める（動画のED画面を作るため）】\n"
    "[ED]\n[EDIMG: 素材key3つ（前向きな締めに合うもの。例 gold_bar, tsumitate, gutspose）]\n"
    "【四国めたん】(結論の一文＋コメント誘導＋チャンネル登録・高評価のお願い。『ご視聴ありがとうございました』を含める)\n"
)
...
PASSES = [
    ("第1部 掴み・対立", True,
     VIDEO_FORMAT_OP +
     "OP（HOOK/TITLE/DESC）に続けてブロック1〜6（1導入/2問題提起/3逆張り登場/4定義/5解説①実質金利[GRAPH:]/6ドル指数）を書く。"
     "価格はここで一度だけ提示する。"),
    ("第2部 種明かし", False,
     "ブロック7〜12（7有事/8インフレ/9中銀買い/10解説②長期チャート[GRAPH:]/11金vsBTC/12金vs米国株）を書く。"
     "# タイトルやOPマーカーや導入は書かない。直前の会話の続きから自然に始め、同じ論点・レスを繰り返さない。"
     "場面の感情が変わるレス直前に [IMG:感情key] を置く（sticky）。"),
    ("第3部 実需オチ", False,
     "ブロック13〜17（13現物vsETFvs積立/14ワイの体験談で山場/15買い時と積立[GRAPH:]/16税金・出口/17結論）を書き、"
     "最後は下記EDフォーマットで締める。# タイトルや導入は書かない。\n" + VIDEO_FORMAT_ED),
]
